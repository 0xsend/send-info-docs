...
import SendVerified from '../components/SendVerified.tsx';
import EcosystemFund from '../components/EcosystemFund.tsx';
import LiquidityPairs from '../components/LiquidityPairs.tsx';
import CUSDOverview from '../components/CUSDOverview.tsx';
import SendConnect from '../components/SendConnect.tsx';
import AssetRelayer from '../components/AssetRelayer.tsx';
...
export async function renderDocToComponent(slug) {
  ...
  const mdx = await compileMDX({
    ...
    options: {
      // blockJS: false is required to render inline JSX style={{...}} expressions
      // in MDX files (used heavily in roadmap, etc). Safe here since all MDX is
      // committed to the repo, not user-supplied.
      blockJS: false,
      mdxOptions: {
        remarkPlugins: [gfm],
        rehypePlugins: [
          // Build-time highlighting via Shiki; no highlighter reaches the client.
          // keepBackground: false so the code-block background stays in
          // globals.css rather than being baked in by the theme.
...
export function buildSidebarTree() {
  ...
  [...tree.keys()].sort().forEach((k) => {
    ...
    if (k === 'pool-party') {
      const order = [
        'pool-party/overview',
        'pool-party/how-it-works',
        'pool-party/liquidity-pairs',
        'pool-party/rewards',
        'pool-party/faqs'
      ];
      const index = new Map(order.map((v, i) => [v, i]));
    ...
    if (k === 'asset-relayer') {
      const order = [
        'asset-relayer/overview',
        'asset-relayer/how-to-bridge',
        'asset-relayer/integration-guide'
      ];
      const index = new Map(order.map((v, i) => [v, i]));
      items.sort((a, b) => (index.has(a.id) ? index.get(a.id) : 999) - (index.has(b.id) ? index.get(b.id) : 999));
    }
    if (k === 'cusd-stablecoin') {
      ...
      const order = [
        ...
        'cusd-stablecoin/buybacks',
        'cusd-stablecoin/partners',
