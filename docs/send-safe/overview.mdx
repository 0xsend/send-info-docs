...
Send Safe is a passkey-first crypto wallet that lets you log in with your face or fingerprint instead of memorizing seed phrases. It runs on Canton Network, so your transactions stay private end to end.

Send Safe is currently a separate app from Send App. Canton Network's technology stack is fundamentally different from EVM-based chains, so we built a dedicated wallet to get it into your hands faster. We plan to consolidate Send Safe into Send App around Q1 2026.
...
Send Safe fixes both problems today: no seed phrases to lose, no public transaction history to expose your financial activity.
...
1. Visit <a href="https://sendsafe.global" className="multisig-token-link" target="_blank" rel="noreferrer">SendSafe.global ↗</a> on a passkey-capable device.
2. Create or select the passkey you want to use (Face ID, Touch ID, Windows Hello, or a hardware security key).
3. Approve the Send Safe enrollment prompt and confirm your recovery device if prompted.
...
Need help with enrollment or device rotation? Reach the Send Safe team through the in-app support widget or email <a href="mailto:support@send.app" className="multisig-token-link">support@send.app</a>.

## Status

Send Safe launched on September 28th, 2025 and is now live. Sign in at <a href="https://sendsafe.global" className="multisig-token-link" target="_blank" rel="noreferrer">SendSafe.global ↗</a> to enroll with your passkey and start transacting.
