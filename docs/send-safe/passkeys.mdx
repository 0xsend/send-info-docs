---
id: send-safe-passkeys
title: Passkeys & Security
---

# Send Safe Passkeys

Passkeys let Send Safe owners authenticate with platform biometrics or hardware security keys instead of seed phrases. Only devices that you explicitly enroll can authorize transactions.

## How Passkeys Work

- Send Safe uses WebAuthn to bind a cryptographic credential to each trusted device.
...
Send Safe is non-custodial. Your passkeys and devices are the only way to authorize movements of $SEND, $CC, or other supported assets. Send cannot access, freeze, or transfer funds on your behalf.
...
- Review the [Send Safe Overview](./overview) for setup guidance before enrolling devices.
