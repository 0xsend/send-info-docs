...
$SEND can be traded within Send and on supported decentralized exchanges on Base.
...
Track everything in Send—your balance, transaction history, and reward distributions are all displayed in real-time.
...
The current $SEND price is displayed in Send and on various crypto tracking platforms.

## Tokenomics

### What is the total supply of $SEND?

For detailed information about $SEND supply, distribution, and tokenomics, please refer to the [Distribution](/docs/send-token/distribution) page.

### Is there a token vesting schedule?

Yes. Different allocation categories have different vesting schedules. See the [Distribution](/docs/send-token/distribution) page for complete details.

### Are there token burns?

The $SEND tokenomics model includes mechanisms for ecosystem sustainability. Check the [Distribution](/docs/send-token/distribution) page for specifics.
