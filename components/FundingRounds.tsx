...
// ============ FUNDING DATA ============
const fundingRounds = [
  { round: 1, label: 'TGE', date: 'June 2023', amount: 340000, price: 0.0034, tokens: 100000000, contributors: 171 },
  { round: 2, label: 'Round 2', date: 'August 2023', amount: 100000, price: 0.01, tokens: 10000000, contributors: 5 },
  { round: 3, label: 'Round 3', date: 'May 2024', amount: 150000, price: 0.03125, tokens: 4800000, contributors: 1 },
  { round: 4, label: 'Round 4', date: 'January 2025', amount: 100000, price: 0.0143, tokens: 7000000, contributors: 2 },
];
...
const avgPrice = totalRaised / totalTokensSold;
...
function FundingChart() {
  ...
  const maxAmount = Math.max(...fundingRounds.map(r => r.amount));
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${w} ${h}`} style={{ overflow: 'visible' }}>
      ...
      {fundingRounds.map((r, i) => {
        ...
        const barH = (r.amount / maxAmount) * ih;
        ...
        return (
          <g key={r.round} onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)} style={{ cursor: 'pointer' }}>
            ...
            <rect
              ...
              fill="#122023" rx="4"
              opacity={dim ? 0.15 : isHov ? 1 : 0.85}
              style={{ transition: 'opacity 0.15s' }}
            />

            {/* Price label above each bar (always visible) */}
            <text x={x + barW / 2} y={pad.top + ih - barH - 8} textAnchor="middle" fontSize="10" fontWeight="700" fill={isHov ? '#1a8a2e' : '#1a8a2e'} fontFamily={MONO} opacity={dim ? 0.4 : 1}>
              {fmtPrice(r.price)}
            </text>

            {/* Tooltip — shows raised and contributors */}
            {isHov && (
              <g>
                <rect x={x + barW / 2 - 70} y={pad.top + ih - barH - 78} width="140" height="44" rx="6" fill="#122023" />
                <text x={x + barW / 2} y={pad.top + ih - barH - 60} textAnchor="middle" fontSize="10" fill="#6b7c7f" fontFamily={MONO}>Raised</text>
                <text x={x + barW / 2} y={pad.top + ih - barH - 44} textAnchor="middle" fontSize="13" fontWeight="700" fill="#FFF" fontFamily={MONO}>{fmt(r.amount)}</text>
              </g>
            )}

            {/* X labels */}
            <text x={x + barW / 2} y={h - 28} textAnchor="middle" fontSize="12" fontWeight="600" fill={isHov ? '#122023' : '#888'} fontFamily={MONO}>
              R{r.round}
            </text>
            <text x={x + barW / 2} y={h - 12} textAnchor="middle" fontSize="9" fill="#bbb" fontFamily={MONO}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div className="bento-hero">
        ...
        <div style={{ background: '#171f22', padding: '24px 28px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          ...
          <div>
            <div style={{ fontFamily: MONO, fontSize: '26px', fontWeight: 700, color: '#FFF', letterSpacing: '-1px' }}>{fmtNum(totalTokensSold)}</div>
            <div style={{ fontSize: '11px', color: '#4a5c5f', marginTop: '4px' }}>{((totalTokensSold / 1_000_000_000) * 100).toFixed(1)}% of supply</div>
          </div>
        </div>

        {/* Avg Price + Contributors */}
        <div style={{ background: '#171f22', padding: '24px 28px', display: 'flex', gap: '24px' }}>
          <div style={{ flex: 1 }}>
            <div style={LABEL}>Avg Price</div>
            <div style={{ fontFamily: MONO, fontSize: '22px', fontWeight: 700, color: '#FFF', marginTop: '6px' }}>{fmtPrice(avgPrice)}</div>
            <div style={{ fontSize: '10px', color: '#4a5c5f', marginTop: '2px' }}>Weighted across all rounds</div>
          </div>
          <div>
      ...
      <div style={{ ...CARD, padding: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '16px' }}>
          <div style={LABEL}>Amount Raised by Round</div>
          <div style={{ display: 'flex', gap: '16px' }}>
            <span style={{ fontSize: '10px', color: '#999', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <span style={{ display: 'inline-block', width: '12px', height: '8px', borderRadius: '2px', background: '#122023' }} />
              Amount Raised
            </span>
            <span style={{ fontSize: '10px', color: '#1a8a2e', fontFamily: MONO }}>
              Price shown above each bar
            </span>
          </div>
        </div>
        <FundingChart />
      ...
      <div style={{ ...CARD, overflow: 'hidden' }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                {['Round', 'Date', 'Raised', 'Price', 'Tokens', 'Contributors'].map((h, i) => (
                  <th key={h} style={{
